...
float runWithoutGraph() {
    ...
    // Synchronize stream2 before using d_arrayB in kernelC
...
float runWithGraph() {
    ...
    CUDA_CHECK(cudaStreamBeginCapture(stream2, cudaStreamCaptureModeGlobal));
    ...
    // Synchronize stream2 before using d_arrayB in kernelC
    CUDA_CHECK(cudaEventRecord(graphCreateStop, stream2));
