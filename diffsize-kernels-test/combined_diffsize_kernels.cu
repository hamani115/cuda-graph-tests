...
void runWithoutGraph(float* totalTimeWith, float* totalTimeWithout) {
    ...
    CUDA_CHECK(cudaMalloc(&d_arrayA, arraySizeA * sizeof(double)));
    CUDA_CHECK(cudaMalloc(&d_arrayB, arraySizeB * sizeof(int)));
    CUDA_CHECK(cudaMalloc(&d_arrayC, arraySizeC * sizeof(double)));
    CUDA_CHECK(cudaMalloc(&d_arrayD, arraySizeD * sizeof(float)));
    CUDA_CHECK(cudaMalloc(&d_arrayE, arraySizeE * sizeof(int)));
    ...
    CUDA_CHECK(cudaFree(d_arrayC));
    CUDA_CHECK(cudaFree(d_arrayD));
    CUDA_CHECK(cudaFree(d_arrayE));
    ...
    for(int i = 0; i < NSTEP; ++i){
        ...
        for (size_t j = 0; j < arraySizeD; j++) {
            h_arrayD[j] = static_cast<f_arrayE[i] = 1;
        }loat>(j) * 0.01f;
        }
