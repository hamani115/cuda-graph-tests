...
#include <cuda.h> // Include for clock64()
...
__global__ void matMulKernel(float* A, float* B, float* C, int width, unsigned long long int* d_kernelTime) {
    // Variables to hold start and end times
    unsigned long long int start = 0, end = 0;

    // Have a single thread measure the time
    ...
    if (threadIdx.x == 0 && threadIdx.y == 0 && blockIdx.x == 0 && blockIdx.y == 0) {
        ...
        d_kernelTime[0] = end - start;
    }
}

// __global__ void matMulKernel(float* A, float* B, float* C, int width, unsigned long long int* d_blockTimes) {
//     //! Calculate block and thread indices
//     int blockId = blockIdx.y * gridDim.x + blockIdx.x; // Unique block index
//     int row = blockIdx.y * blockDim.y + threadIdx.y;
//     int col = blockIdx.x * blockDim.x + threadIdx.x;

//     // Declare shared memory for per-block timing accumulation
//     __shared__ unsigned long long int blockTime;

//     // Initialize shared timing variable
...
//     // Atomically add thread time to block time
//     atomicAdd(&blockTime, threadTime);

//     __syncthreads();

//     // Store block time in global memory
//     if (threadIdx.x == 0 && threadIdx.y == 0) {
//         d_blockTimes[blockId] = blockTime;
//     }
// }
...
void matrixMultiplyNoGraph(int width, float* totalTimeWith, float* totalTimeWithout) {
    ...
    // Allocate device memory for per-block timing data
    unsigned long long int* d_blockTimes;
    CUDA_CHECK(cudaMalloc(&d_blockTimes, numBlocks * sizeof(unsigned long long int)));

    // Allocate host memory for timing data
    unsigned long long int* h_blockTimes = (unsigned long long int*)malloc(numBlocks * sizeof(unsigned long long int));

    // Get GPU clock frequency
    cudaDeviceProp prop;
    CUDA_CHECK(cudaGetDeviceProperties(&prop, 0));
    std::cout << "Clock Rate: " << prop.clockRate << std::endl;
    double clockFrequency = prop.clockRate * 1000.0; // Convert kHz to Hz
    ...
    for (int i = 0; i < NKERNEL; i++) {
        // Reset timing data on device
        std::cout << "Clock Rate: " << prop.clockRate << std::endl;
        CUDA_CHECK(cudaMemset(d_blockTimes, 0, numBlocks * sizeof(unsigned long long int)));

        // Launch the kernel
        matMulKernel<<<grid, block, 0, stream>>>(d_A, d_B, d_C, width, d_blockTimes);
        CUDA_CHECK(cudaGetLastError());
        CUDA_CHECK(cudaStreamSynchronize(stream));

        // Copy timing data back to host
        CUDA_CHECK(cudaMemcpy(h_blockTimes, d_blockTimes, numBlocks * sizeof(unsigned long long int), cudaMemcpyDeviceToHost));

        // Process timing data
        unsigned long long int totalTimeCycles = 0;
        for (int j = 0; j < numBlocks; j++) {
            totalTimeCycles += h_blockTimes[j];
        }

        double totalTimeSeconds = totalTimeCycles / clockFrequency;

        // Print timing per kernel
        std::cout << "First Run Kernel " << i << " Addition Time: " << totalTimeCycles << " cycles (" << totalTimeSeconds << " s)" << std::endl;
    }
    ...
    for (int j = 0; j < NSTEP - 1; j++) {
        ...
        for (int i = 0; i < NKERNEL; i++) {
            ...
            for (int k = 0; k < numBlocks; k++) {
                totalTimeCycles += h_blockTimes[k];
            }

            double totalTimeSeconds = totalTimeCycles / clockFrequency;

            // Print timing per kernel
            std::cout << "Iteration " << j + 1 << ", Kernel " << i + 1 << " Addition Time: " << totalTimeCycles << " cycles (" << totalTimeSeconds << " s)" << std::endl;
        }
    ...
    // Cleanup
    free(h_blockTimes);
    CUDA_CHECK(cudaFree(d_blockTimes));
